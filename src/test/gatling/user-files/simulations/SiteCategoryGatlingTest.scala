...
/**
 * Performance test for the SiteCategory entity.
 */
class SiteCategoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SiteCategory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all siteCategories")
            .get("/api/site-categories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new siteCategory")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_siteCategory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created siteCategory")
                .get("${new_siteCategory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created siteCategory")
            .delete("${new_siteCategory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
