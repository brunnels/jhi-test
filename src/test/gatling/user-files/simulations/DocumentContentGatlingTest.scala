...
/**
 * Performance test for the DocumentContent entity.
 */
class DocumentContentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocumentContent entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all documentContents")
            .get("/api/document-contents")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new documentContent")
            .post("/api/document-contents")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "data":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_documentContent_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created documentContent")
                .get("${new_documentContent_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created documentContent")
            .delete("${new_documentContent_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
